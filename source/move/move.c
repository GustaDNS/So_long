...
/*   Updated: 2024/10/08 12:08:15 by gudaniel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_to_up(t_game *game)
{
	if	(game->map.map[game->player.y - 1][game->player.x] == '1')
		return ;
	if	(game->map.map[game->player.y - 1][game->player.x] == 'E' && game->collected != game->collectible)
		return ;
	if (game->map.map[game->player.y - 1][game->player.x] == 'E' && game->collected == game->collectible)
	{
		ft_printf("%d\n",++game->count);
	...
	mlx_put_image_to_window(game->mlx_ptr, game->window_ptr, game->linker.path,
...
void	move_to_left(t_game *game)
{
	if	(game->map.map[game->player.y][game->player.x - 1] == '1')
		return ;
	if	(game->map.map[game->player.y][game->player.x - 1] == 'E' && game->collected != game->collectible)
		return ;
	if (game->map.map[game->player.y][game->player.x - 1] == 'E' && game->collected == game->collectible)
	{
		ft_printf("%d\n",++game->count);
